...
vim.keymap.set('v', '<M-j>', ":m '>+1<cr>gv=gv", { desc = 'Lines: move down' })
vim.keymap.set('v', '<M-k>', ":m '<-2<cr>gv=gv", { desc = 'Lines: move up' })
...
-- Tabs
vim.keymap.set({ 'n', 'v' }, '<leader>+', '<cmd>tabnew<cr>', { desc = 'Tab: new' })
vim.keymap.set({ 'i', 'n', 'v', 't' }, '<M-v>', '<cmd>tabprevious<cr>', { desc = 'Tab: previous' })
...
for idx, char in ipairs { 'z', 'x', 'c' } do
  vim.keymap.set({ 'n', 'v' }, string.format('<M-%s>', char), string.format('%sgt', idx), { desc = 'Tab: switch to tab ' .. idx })
end
